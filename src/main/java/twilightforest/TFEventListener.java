...
public class TFEventListener {
    ...
    /**
     * Check to see if a smeltable block has dropped with a fiery tool, and if so, smelt it Also check
     * if we need to transform 64 cobbles into a giant cobble
     */
    @SubscribeEvent
    ...
    /**
     * We wait for a player wearing armor with the fire react enchantment to get hurt, and if that
     * happens, we react
     */
    @SubscribeEvent
    ...
    /**
     * If a player dies with a charm of keeping, consume the charm and then keep track of what items we
     * need to keep
     * 
    ...
    public void breakBlock(BreakEvent event) {
        if (!event.getPlayer().capabilities.isCreativeMode && isAreaProtected(event.world, event.getPlayer(), event.x, event.y, event.z)
            ...
            event.setCanceled(true);
        } else if (!this.isBreakingWithGiantPick && event.getPlayer().getCurrentEquippedItem() != null && event.getPlayer().getCurrentEquippedItem().getItem() == TFItems.giantPick
    ...
    /**
     * Stop the player from interacting with blocks that could produce treasure or open doors in a
     * protected area
     */
    private boolean isBlockProtectedFromInteraction(World world, int x, int y, int z) {
        Block block = world.getBlock(x, y, z);

        if (block == TFBlocks.towerDevice || block == Blocks.chest || block == Blocks.trapped_chest || block == Blocks.stone_button || block == Blocks.wooden_button || block == Blocks.lever) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * Return if the area at the coordinates is considered protected for that player. Currently, if we
     * return true, we also send the area protection packet here.
     */
    ...
    public void livingAttack(LivingAttackEvent event) {
        ...
        if (event.entityLiving instanceof IMob && event.source.getEntity() instanceof EntityPlayer && !((EntityPlayer) event.source.getEntity()).capabilities.isCreativeMode
                ...
                && event.entityLiving.worldObj.getGameRules().getGameRuleBooleanValue(TwilightForestMod.ENFORCED_PROGRESSION_RULE)) {
