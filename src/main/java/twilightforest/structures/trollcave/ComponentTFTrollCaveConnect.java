...
public class ComponentTFTrollCaveConnect extends ComponentTFTrollCaveMain {
    ...
    public void buildComponent(StructureComponent parent, List<StructureComponent> list, Random rand) {
        ...
        if (this.getComponentType() < 3) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (rand.nextBoolean() || !makeGardenCave(
                        list,
                        rand,
    ...
    private void makeSingleStoneFormation(World world, StructureBoundingBox sbb, Random decoRNG, int rotation, int z,
        ...
        // this.randomlyFillBlocksRotated(world, sbb, decoRNG, 0.5F, size - (depth + 1), y - width, z -
        // width, size - 1, y + width, z + width, TFBlocks.trollSteinn, 0, Blocks.stone, 0, rotation);
    }
    ...
    private void decorateBracketMushrooms(World world, StructureBoundingBox sbb, Random decoRNG, int rotation) {
        ...
        for (int y = startY; y < this.height; y += 2) {
            ...
            Block mushBlock = ((decoRNG.nextInt(3) == 0) ? TFBlocks.hugeGloomBlock
                    : (decoRNG.nextBoolean() ? Blocks.brown_mushroom_block : Blocks.red_mushroom_block));
     ...
     */
    private void makeSingleBracketMushroom(World world, StructureBoundingBox sbb, int rotation, int z, int y, int width,
            int depth, Block mushBlock) {
    ...
    protected boolean makeGardenCave(List<StructureComponent> list, Random rand, int index, int x, int y, int z,
        ...
        ComponentTFTrollCaveMain cave = new ComponentTFTrollCaveGarden(
                index,
                dest.posX,
                dest.posY,
                dest.posZ,
    ...
    private StructureComponent findNearbyGarden(List<StructureComponent> list, StructureBoundingBox boundingBox) {
        ...
        for (StructureComponent component : list) {
            if (component instanceof ComponentTFTrollCaveGarden
                    && component.getBoundingBox().intersectsWith(largeBox)) {
