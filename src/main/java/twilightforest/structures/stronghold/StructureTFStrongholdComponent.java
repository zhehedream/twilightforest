...
public abstract class StructureTFStrongholdComponent extends StructureTFComponent {
    ...
    public boolean attemptToBreakIn(int wx, int wy, int wz) {
        if (!isValidBreakInPoint(wx, wy, wz)) {
            ...
            // this.boundingBox.minX, this.boundingBox.maxX, this.boundingBox.minZ, this.boundingBox.maxZ);
    ...
    public void placeDoors(World world, Random rand, StructureBoundingBox sbb) {
        if (this.doors != null) {
            for (ChunkCoordinates doorCoords : doors) {
                ...
                // this.placeBlockAtCurrentPosition(world, Blocks.wool, doorCoords.posX, doorCoords.posX,
                // doorCoords.posY + 2, doorCoords.posZ, sbb);
            }
        }
    }
