...
public class BlockTFDoor extends Block {
    ...
    protected BlockTFDoor(int doorType) {
        super(Material.wood);
        float f = 0.5F;
        float f1 = 1.0F;
        this.setBlockBounds(0.5F - f, 0.0F, 0.5F - f, 0.5F + f, f1, 0.5F + f);
        this.material = doorType % names.length;
    }

    protected BlockTFDoor() {
        super(Material.wood);
        float f = 0.5F;
        float f1 = 1.0F;
        this.setBlockBounds(0.5F - f, 0.0F, 0.5F - f, 0.5F + f, f1, 0.5F + f);
    ...
    public void registerBlockIcons(IIconRegister reg) {
        ...
        this.field_150017_a[0] = reg.registerIcon(TwilightForestMod.ID + ":wood/door/" + names[material] + "_upper");
        this.field_150016_b[0] = reg.registerIcon(TwilightForestMod.ID + ":wood/door/" + names[material] + "_lower");
    ...
    public boolean getBlocksMovement(IBlockAccess worldIn, int x, int y, int z) {
        int l = this.func_150012_g(worldIn, x, y, z);
        return (l & 4) != 0;
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess worldIn, int x, int y, int z) {
        this.func_150011_b(this.func_150012_g(worldIn, x, y, z));
    }
    ...
    public void onNeighborBlockChange(World worldIn, int x, int y, int z, Block neighbor) {
        ...
        if ((l & 8) == 0) {
            ...
            if (flag) {
                ...
                boolean flag1 = worldIn.isBlockIndirectlyGettingPowered(x, y, z)
    ...
    public boolean canPlaceBlockAt(World worldIn, int x, int y, int z) {
        return y >= worldIn.getHeight() - 1 ? false
                : World.doesBlockHaveSolidTopSurface(worldIn, x, y - 1, z) && super.canPlaceBlockAt(worldIn, x, y, z)
    ...
    public Item getItem(World worldIn, int x, int y, int z) {
        ...
        switch (material) {
            default:
                door = TFItems.doorTwilight;
                break;
            case 1:
                door = TFItems.doorCanopy;
                break;
            case 2:
                door = TFItems.doorMangrove;
                break;
            case 3:
