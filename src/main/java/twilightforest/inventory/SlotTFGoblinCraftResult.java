...
public class SlotTFGoblinCraftResult extends SlotCrafting {
    ...
    public void onPickupFromSlot(EntityPlayer par1EntityPlayer, ItemStack par1ItemStack) {
        // let's see, if the assembly matrix can produce this item, then it's a normal recipe, if not, it's
        ...
        if (combined) {
            ...
            if (this.uncraftingMatrix.recraftingCost > 0) {
                ...
                // System.out.println("Charging the player " + this.uncraftingMatrix.recraftingCost + " experience
                // for recrafting");
            }

            // if we are using a combined recipe, wipe the uncrafting matrix and decrement the input
            // appropriately
